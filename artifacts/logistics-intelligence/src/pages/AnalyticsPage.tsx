...
function DarkTooltip({
  ...
  return (
    <div className="min-w-[132px] rounded-md border border-slate-700 bg-slate-100 px-3 py-2 text-xs text-slate-900 shadow-xl">
      ...
      {payload.map((entry) => (
        <div className="flex items-center justify-between gap-4" key={`${entry.name}-${entry.value}`}>
          ...
          <span className="font-mono font-semibold tabular-nums">{entry.value ?? 0}</span>
        </div>
      ))}
    </div>
  );
